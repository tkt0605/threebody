...
// 副体は「視点を出す係」、主体は「作る係」。役割を分けないと、副体2体が同じ具体物を
// 別々に書き、主体がそれを棄却して終わる（実測: WebSocketアプリを尋ねたら副体2体が
// どちらも動かないTCPソケットのコードを書き、主体は正しく棄却したうえで何も出さなかった）。
// 副体には secondaryMaxTokens しか与えていないので、そもそも具体物は途中で切れる
const NO_ARTIFACT = `
コードや具体的な実装そのものは書かない。
...
リスクを無視するのではなく、それでも前に進む理由や、うまくいく場合に何が得られるかを具体的に示す。${NO_ARTIFACT}`,
  },
  skeptic: {
    name: '懐疑的な視点',
    personaPrompt: `【担当する視点】懐疑主義者
見落とされがちな前提・リスク・失敗のシナリオを指摘する。
悲観のための悲観ではなく、具体的にどこで計画が崩れうるかを示す。${NO_ARTIFACT}`,
  },
  realist: {
    name: '現実的な視点',
    personaPrompt: `【担当する視点】現実主義者
感情論や極端な楽観・悲観を避け、実際に取りうる選択肢とその現実的なコスト・制約を整理する。
理想と懸念の両方を踏まえた上で、地に足のついた判断材料を提示する。${NO_ARTIFACT}`,
  },
}
...
- テキスト入力が無く、入力手段が音声だけ（棚卸し 1.1「最優先」／外部レビュー #11 と同根）
- UIの日本語ハードコード（棚卸し 1.11）／モデル名が手入力（1.12）
- `messages.content` と `content_blocks` の二重保存（3.10）／E2E・ブラウザ互換の検証が無い（2.11）
- 中断した応答は永続化されないため、リロードすると消える（ユーザー発言だけが残り「もう一度送信」導線になる）

**トラックA：音声（Phase 2）**

- 逐次TTS（#4）
- バージイン（#5）— `echoCancellation` の明示が済んだので前提条件は揃った
- エンドポインティング調整（#7）
- ※ 三体モードと音声の緊張（A:主体先行 / B:音声時縮退 / C:球体+相槌）の方針決定が未了で、これが着手のブロッカー

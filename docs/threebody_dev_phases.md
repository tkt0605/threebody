...
作成日: 2026-08-06 / 最終更新: 2026-08-14
...
- テキスト入力が無く、入力手段が音声だけ（棚卸し 1.1「最優先」／外部レビュー #11 と同根）。`ChatView.vue` にチャット用の `input`/`textarea` が0件で、Firefoxでは `useVoiceInput.ts:108` の「このブラウザは音声認識に対応していません」で行き止まりになる
...
- 利用規約・プライバシーポリシーが存在しない（`LoginView.vue:68` は同意を求めているが、規約自体が無い）／退会（アカウント削除）の導線が無い
...
**未適用のまま残っているもの** — `docs/schema.sql` 6章**ブロックC**（`release_global_quota`）。
2026-08-11 から状態が変わっておらず、適用の記録も無い。未適用なら失敗したぶんの全体枠が
戻らないため、キルスイッチ（50回/日）が実質的に目減りし続ける。**Supabase側の確認が必要**。
